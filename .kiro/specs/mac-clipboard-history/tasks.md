...
- [x] 2. Data Models Implementation
  - [x] 2.1 Implement `ClipboardContent` enum with `.text(String)` and `.image(Data)` cases, `Codable` and `Equatable` conformance, `textPreview` computed property, and `exceedsSizeLimit` check
  - [x] 2.2 Implement `ClipboardEntry` struct with `id: UUID`, `content: ClipboardContent`, `timestamp: Date`, `contentType: ContentType` enum, and `Codable`/`Equatable`/`Identifiable` conformance
  - [x] 2.3 Implement `KeyboardShortcut` struct with `keyCode`, `modifiers`, `isValid` computed property, and `static let default` for Cmd+Shift+V
  - [x] 2.4 Implement `RelativeTimestampFormatter` that formats dates using the largest applicable unit (days >= 86400s, hours >= 3600s, minutes >= 60s, seconds otherwise)
  - [x] 2.5 Implement image thumbnail scaling utility function that fits dimensions within 64x64 while preserving aspect ratio

- [x] 3. Property Tests for Data Models
  - [x] 3.1 Write property test for serialization round-trip (Property 1): generate random ClipboardEntry values, encode to JSON, decode back, assert equality **Validates: Requirements 1.1, 1.2**
  - [x] 3.2 Write property test for size limit enforcement (Property 3): generate strings/data of varying sizes, verify boundary at 1,000,000 chars and 10 MB **Validates: Requirements 1.8**
  - [x] 3.3 Write property test for text preview formatting (Property 9): generate random strings with newlines and varying lengths, verify truncation and newline replacement rules **Validates: Requirements 4.2**
  - [x] 3.4 Write property test for image thumbnail scaling (Property 10): generate random dimensions, verify output fits 64x64 with correct aspect ratio within +-1px tolerance **Validates: Requirements 4.3**
  - [x] 3.5 Write property test for relative timestamp formatting (Property 11): generate random time intervals, verify largest applicable unit selection **Validates: Requirements 4.4**
  - [x] 3.6 Write property test for shortcut validation (Property 8): generate random key+modifier combinations, verify validation logic **Validates: Requirements 3.5**

- [x] 4. HistoryStore Implementation
  - [x] 4.1 Implement `HistoryStore` actor with in-memory `entries` array and `maxEntries = 50`
  - [x] 4.2 Implement `addEntry(_:)` with FIFO eviction when at capacity (remove oldest before adding new)
  - [x] 4.3 Implement `deleteEntry(id:)` to remove a single entry by UUID
  - [x] 4.4 Implement `clearAll()` to remove all entries
  - [x] 4.5 Implement `moveToTop(id:)` to move a selected entry to the most recent position
  - [x] 4.6 Implement `getAllEntries()` returning entries ordered most recent first
  - [x] 4.7 Implement `search(query:)` with case-insensitive substring matching on text entries, excluding image entries
  - [x] 4.8 Implement JSON persistence: `load()`, `schedulePersist()` (debounced within 1s), and `persistImmediately()`
  - [x] 4.9 Implement storage URL resolution to `~/Library/Application Support/ClipboardHistory/clipboard_history.json`
  - [x] 4.10 Implement corrupted file recovery: rename to `.corrupt`, start empty, post user notification

- [x] 5. Property Tests for HistoryStore
  - [x] 5.1 Write property test for duplicate detection correctness (Property 2): generate random content, verify detection for identical/different content against most recent entry **Validates: Requirements 1.5**
  - [x] 5.2 Write property test for capacity invariant with FIFO eviction (Property 4): generate sequences of >50 entries, verify store never exceeds 50 and oldest is evicted **Validates: Requirements 2.2, 2.3**
  - [x] 5.3 Write property test for ordering invariant (Property 5): generate random operation sequences, verify entries always ordered most recent first **Validates: Requirements 2.6, 1.3**
  - [x] 5.4 Write property test for deletion removes exactly one entry (Property 6): generate stores of random sizes, delete random entries, verify correct removal and order preservation **Validates: Requirements 2.4**
  - [x] 5.5 Write property test for move-to-top preserves relative order (Property 7): generate stores, move random entries, verify moved entry is first and others maintain order **Validates: Requirements 5.3**
  - [x] 5.6 Write property test for search filtering correctness (Property 13): generate random entry sets and queries, verify only matching text entries returned **Validates: Requirements 7.3, 7.4**
  - [x] 5.7 Write property test for search clear restores full list (Property 14): generate entries, filter, clear, verify full list restored **Validates: Requirements 7.6**

- [-] 6. ClipboardMonitor Implementation
  - [x] 6.1 Implement `ClipboardMonitor` actor with reference to `NSPasteboard.general` and `lastChangeCount` tracking
  - [x] 6.2 Implement `startMonitoring()` with a Timer firing every 500ms calling `checkForChanges()`
  - [x] 6.3 Implement `checkForChanges()` comparing `pasteboard.changeCount` to `lastChangeCount` and processing new content
  - [x] 6.4 Implement `extractText()` to read plain text or rich text from the pasteboard
  - [x] 6.5 Implement `extractImage()` to read image data (PNG/TIFF) from the pasteboard
  - [x] 6.6 Implement `isDuplicate(_:)` to compare new content against the most recent entry (byte-identical check)
  - [x] 6.7 Implement size limit enforcement: discard text > 1,000,000 chars or image > 10 MB
  - [x] 6.8 Implement error handling: skip cycle on pasteboard access failure without interrupting monitoring
...
- **Environment constraint:** The development environment is Windows — do NOT run any Swift/`swift build`/`swift test` commands. Validate implementation by file inspection and diagnostics only. This restriction is in effect until explicitly communicated otherwise.
